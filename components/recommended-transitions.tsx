...
export default function RecommendedTransitions() {
  ...
  const [recommendations, setRecommendations] = useState<TransitionWithRatings[]>([])
      ...
      try {
        ...
        userRatingsData.forEach((rating: any) => {
          if (rating.transitions) {
            if (rating.transitions.song1_artist) likedArtists.add(rating.transitions.song1_artist)
            if (rating.transitions.song2_artist) likedArtists.add(rating.transitions.song2_artist)
          }
        })
        ...
        // Create a more reliable query approach
        let query = supabase
          .from("transitions")
          .select(`
            *,
        ...
        // Only apply artist filter if we have liked artists
        if (likedArtistsArray.length > 0) {
          // Build a filter condition for each artist
          const artistFilters = likedArtistsArray.map((artist) => {
            return `song1_artist.ilike.%${artist}%,song2_artist.ilike.%${artist}%`
          })

          // Join the conditions with commas for the OR query
          const filterString = artistFilters.join(",")
        ...
        const { data: recommendedTransitions, error: recommendError } = await query

        if (recommendError) {
          console.error("Error fetching recommended transitions:", recommendError)
        ...
        const ratedTransitionIds = new Set(userRatingsData.map((r: any) => r.transition_id).filter(Boolean))
        ...
        if (filteredRecommendations.length > 0) {
          ...
          console.log("No filtered recommendations, showing recent transitions")
          setRecommendations(recentTransitions)
        }
      } catch (error: any) {
  ...
  if (recommendations.length === 0) {
    return (
      <div className="text-center py-4">
        <p className="text-muted-foreground">No recommendations available yet. Try rating more transitions!</p>
      </div>
    )
  }
