...
export default async function FeaturedTransitions() {
  ...
  // Fetch featured transitions (most upvoted)
              ...
              <CardContent className="p-0">
                <div className="flex items-center gap-2 bg-muted p-3">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium truncate">{transition.song1_name}</p>
                    <p className="text-xs text-muted-foreground truncate">{transition.song1_artist}</p>
                  </div>
                  <div className="text-center text-xs font-medium">to</div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium truncate">{transition.song2_name}</p>
                    <p className="text-xs text-muted-foreground truncate">{transition.song2_artist}</p>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2 p-3">
                  <img
                    src={transition.song1_image || "/placeholder.svg?height=120&width=120"}
                    alt={transition.song1_name}
                    className="aspect-square w-full rounded object-cover"
                  />
                  <img
                    src={transition.song2_image || "/placeholder.svg?height=120&width=120"}
