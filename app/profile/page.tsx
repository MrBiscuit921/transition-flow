      ...
      <div className="mx-auto max-w-4xl">
              ...
              <div>
                ...
                <CardDescription>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span>Member since {new Date(session.user.created_at).toLocaleDateString()}</span>
                  </div>
                </CardDescription>
              </div>
            </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="flex flex-col items-center justify-center rounded-lg border bg-card p-4 shadow-sm">
                <div className="text-3xl font-bold">{userTransitions.length}</div>
                <div className="text-sm text-muted-foreground">Transitions Shared</div>
              </div>
              <div className="flex flex-col items-center justify-center rounded-lg border bg-card p-4 shadow-sm">
                <div className="text-3xl font-bold">{userRatings.filter(r => r.rating > 0).length}</div>
                <div className="text-sm text-muted-foreground">Upvotes Given</div>
              </div>
              <div className="flex flex-col items-center justify-center rounded-lg border bg-card p-4 shadow-sm">
                <div className="text-3xl font-bold">{userRatings.filter(r => r.rating < 0).length}</div>
                <div className="text-sm text-muted-foreground">Downvotes Given</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Tabs defaultValue="transitions">
          ...
          <TabsContent value="transitions">
            <h2 className="text-xl font-bold mb-4">Transitions You've Shared</h2>
            {isLoading ? (
                            ...
                            <div className="flex flex-col justify-center">
                              ...
                              <div className="mt-2 flex flex-wrap items-center gap-2">
                                ...
                                <div className="flex items-center gap-1 text-sm text-muted-foreground">
                                  ...
                                  <span>{formatDistanceToNow(new Date(transition.created_at), { addSuffix: true })}</span>
                                </div>
                              </div>
                            </div>
              ...
              <div className="rounded-lg border bg-card p-8 text-center">
                ...
                <h3 className="mt-4 text-lg font-medium">No transitions shared yet</h3>
                <p className="mt-2 text-sm text-muted-foreground">
                  You haven't shared any transitions yet. Start by submitting your first transition!
                </p>
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {userRatings.map((rating) => {
                  ...
                  return (
                    <Link key={rating.id} href={`/transitions/${transition.id}`}>
                      <Card className="overflow-hidden transition-all hover:shadow-md">
                        <CardContent className="p-4">
                          <div className="grid gap-4 sm:grid-cols-[auto_1fr_auto]">
                            <div className="flex items-center gap-2">
                            ...
                            <div className="flex flex-col justify-center">
                              ...
                              <div className="mt-2 flex flex-wrap items-center gap-2">
                                <Badge variant="outline">{transition.crossfade_length}s crossfade</Badge>
                                <Badge variant={rating.rating > 0 ? "default" : "destructive"}>
              ...
              <div className="rounded-lg border bg-card p-8 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  You haven't rated any transitions yet. Browse transitions and start rating!
                </p>
                <Button asChild className="mt-4">
